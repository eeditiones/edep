...
declare function rview:people-all($request as map(*)) {
    let $people :=
        ...
        collection($config:data-root || "/registers/people")//tei:person
...
declare function rview:people-categories($request as map(*)){
    ...
    let $people :=
        if ($search and $search != '') then
            ...
            collection($config:data-root || "/registers/people")//tei:person[ft:query(., 'name:(' || $search || '*)')]
            
        else
            
            collection($config:data-root || "/registers/people")//tei:person[ft:query(., '*', map {
...
declare function rview:places($request as map(*)){
    ...
    let $places :=
        if ($search and $search != '') then
            ...
            collection($config:data-root || "/registers/places")//tei:place[ft:query(., 'name:(' || $search || '*)')]
            
        else
            
            collection($config:data-root || "/registers/places")//tei:place
